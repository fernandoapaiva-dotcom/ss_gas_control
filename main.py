...
def get_marcas(db: Session = Depends(get_db)):
    marcas = db.query(models.CilindroAplicado.marca).filter(models.CilindroAplicado.marca != None, models.CilindroAplicado.marca != "").distinct().all()
...
async def create_entrega(
    ...
    cnpj = payload.get('cnpj')
    nome_cliente = payload.get('nome_cliente', '').strip()
    if not nome_cliente:
...
async def filtrar_entregas(
    ...
    for e in entregas:
        ...
        result.append({
            ...
            "itens": [{"gas": i.tipo_gas, "tam": i.tamanho_gas, "qtd": i.quantidade, "obs": i.observacao, "marca": i.marca, "validade": i.data_validade} for i in e.cilindros]
        })
    return result
